...
p_test = np.asarray([np.sum((1.0/np.sqrt(2*math.pi) * np.exp(-0.5*(x -x_train)**2 /bin_width **2)) * y_train) / np.sum((1.0/np.sqrt(2*math.pi) * np.exp(-0.5*(x -x_train)**2 /bin_width **2))) for x in x_test])
...
square_error = np.sqrt(np.sum(np.square(y_test - p_test))/len(y_test))
print("Kernel smoother => RMSE is " + str(square_error) +" when h is " + str(bin_width))
